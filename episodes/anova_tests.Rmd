...
> **Example (one factor):** Does commute time (D1) differ across age groups
(18–34, 35–54, 55+)?

> **Example (two factors):** Does commute time (D1) differ across age group *and* gender — tests both main effects and their interaction.
...
> **Example:** Measuring anxiety at three time points (*within*) across a treatment vs control group (*between*).
...
|---|---|
| One-way ANOVA, equal group sizes | Tukey HSD |
| One-way ANOVA, unequal group sizes | Bonferroni correction |
| Welch's ANOVA | **Games-Howell** (does not assume equal variances) |
| Two-way ANOVA, interaction present | Simple effects analysis, then Bonferroni |
| Repeated measures ANOVA | Pairwise comparisons with Bonferroni adjustment |
| Mixed ANOVA | Separate Bonferroni-corrected comparisons per factor |
...
| Test | Factors | Design | Equal variances required? |
|---|---|---|---|
| One-way ANOVA | 1 | Between-subjects | Yes |
| **Welch's ANOVA** | 1 | Between-subjects | **No** |
| Two-way ANOVA | 2 | Between-subjects | Yes |
| Repeated measures ANOVA | 1+ | Within-subjects | Yes (+ sphericity) |
| Mixed ANOVA | 2+ | Within + between | Yes (+ sphericity) |
| Kruskal-Wallis | 1 | Between-subjects | No (non-parametric) |
| Friedman | 1 | Within-subjects | No (non-parametric) |
...
**Test 2: **Research question:** Does commute time (D1) differ across level of education?
(primary, secondary, tertiary or higher)?
...
Rspondents with a primary education appear to have slightly higher commute times, let's continue to see if the differences are statistically significant.
...
The mean daily communte time decreases with age; 18–34: 89.8 min, 35–54: 58.9 min, 55+: 27.2 min. But the `F-test` alone cannot tell us whether all three pairwise differences are significant. We need a post-hoc test. For `Welch's ANOVA`, we can use the `Games-Howell` post-hoc test.
...
The `Games-Howell` post-hoc test reveals that the mean commute times of all age groups are statistically significantly different from each other, and the ges value confirms the effct size is very large. Putting this all together:

"A `Welch's one-way ANOVA` was conducted after after equal variances were not confirmed by `Levenes test`. The results revealed a statistically significant effect of age group on daily commute time, F(2, 263.11) = 695.1, p < 0.001, with mean commute times decreasing across age groups (18–34: mean = 89.8 minutes, std = 17.2; 35–54: mean = 58.9 minutes, std = 20.0; 55+: mean = 27.2 minutes, std = 16.4). 
...
including the F-statistic and effect size. A small GES adds useful information:
the predictor contributes essentially nothing to variance in the dependent variable.
:::
...
**Research question:** Is commute time influenced by gender (A1), age group (A2), or their interaction?
...
The lines here cross, and are parallel: Commute time decreasees with age, and there appears to be an interaction effect. 


### Check eqal variances
```{r}
survey %>%
  levene_test(D1 ~ A1 * A2)
```

Note, if group sizes are roughly equal, `two-way ANOVA` is considered robust to mild violations of homogeneity. We'll continue running the `two-way ANOVA`, but we'll note the violation.

Let's check the group sizes:
...
survey %>%
  ...
  summarise(n = n())
```
Cell sizes are unequal, so we'll note this in the results. Type II sum of squares (used in the `two-way ANOVA) is the appropriate choice for unbalanced designs.
...
-   **Interaction (A1 × A6)**: F(2, 999) = 3.611, p = 0.027, ges = 0.007 (medium effect)
...
## Part 4: Multi-column analysis — across all satisfaction items

So far we have tested one outcome variable at a time. In practice, we often
want to apply the same test to multiple related columns simultaneously.
Here we loop over all three satisfaction items (C1, C2, C3) and compare
their means by gender (A1).

### 4.1 Independent t-tests: all C variables by gender

```{r multi-ttest}
c_vars <- c("C1", "C2", "C3")

results_C_gender <- map_dfr(c_vars, function(var) {
  test_result <- t.test(survey[[var]] ~ survey$A1)

  tibble(
    variable   = var,
    mean_female = test_result$estimate["mean in group female"],
    mean_male   = test_result$estimate["mean in group male"],
    t_stat     = round(test_result$statistic, 3),
...
When running the same test on multiple related variables, we apply the
**Benjamini-Hochberg FDR correction** (`p.adjust(method = "fdr")`). This
controls the expected proportion of false positives among all significant
results, striking a better balance between sensitivity and specificity than
the more conservative Bonferroni method.
:::

### 4.2 Add Cohen's *d* to the loop

```{r multi-ttest-d}
results_C_gender_d <- map_dfr(c_vars, function(var) {
  d_result <- cohens_d(as.formula(paste(var, "~ A1")), data = survey)
...
left_join(results_C_gender, results_C_gender_d, by = "variable")
```

All three C variables show a significant gender difference after FDR
correction, with large effect sizes for C1 and C2, and a medium effect for C3.

### 4.3 One-way ANOVA: all C variables by age group (A2)

```{r multi-anova}
results_C_age <- map_dfr(c_vars, function(var) {
  aov_result <- aov(as.formula(paste(var, "~ A2")), data = survey)
  aov_sum    <- summary(aov_result)
  eta        <- eta_squared(aov_result, partial = FALSE)

  tibble(
    variable   = var,
    F_stat     = round(aov_sum[[1]][["F value"]][1], 3),
    df_between = aov_sum[[1]][["Df"]][1],
    df_within  = aov_sum[[1]][["Df"]][2],
    p_value    = round(aov_sum[[1]][["Pr(>F)"]][1], 4),
    eta_sq     = round(eta$Eta2, 4)
  )
}) %>%
  mutate(
    p_adj       = round(p.adjust(p_value, method = "fdr"), 4),
...
### 4.4 Visualise all effect sizes together

```{r multi-effectsize-plot, fig.width=8, fig.height=5}
# Combine Cohen's d (t-tests) and eta-squared (ANOVAs) in one summary figure
bind_rows(
  results_C_gender_d %>%
    select(variable, effect_size = cohens_d) %>%
    mutate(test = "t-test (gender), Cohen's d"),
  results_C_age %>%
    select(variable, effect_size = eta_sq) %>%
    mutate(test = "ANOVA (age group), η²")
) %>%
  ggplot(aes(x = variable, y = effect_size, fill = test)) +
  geom_col(position = "dodge", alpha = 0.8) +
  labs(title = "Effect Sizes for Satisfaction Items (C1–C3)",
       subtitle = "By gender (Cohen's d) and age group (η²)",
       x = "Satisfaction Item", y = "Effect Size",
       fill = "Test") +
...
Gender produces consistent large effects across all three satisfaction items.
Age group effects are small to negligible.

---

## Summary

| Test | When to use | Key function | Effect size |
|------|-------------|--------------|-------------|
| One-sample t-test | Compare one mean to a fixed value | `t.test(x, mu = …)` | Cohen's *d* |
| Independent t-test | Compare means of two independent groups | `t.test(y ~ group)` | Cohen's *d* |
| Paired t-test | Compare two related measurements | `t.test(x, y, paired = TRUE)` | Cohen's *d* |
| One-way ANOVA | Compare means of ≥3 groups | `aov(y ~ group)` | η² |
| Two-way ANOVA | Two factors ± interaction | `aov(y ~ A * B)` | η² (partial) |
| Tukey HSD | Post-hoc after significant ANOVA | `TukeyHSD(aov_obj)` | — |

**Assumption checking summary**

| Assumption | Check with |
|------------|------------|
| Normality (small samples) | `shapiro_test()`, Q-Q plot |
| Equal variances | `levene_test()` |
| Independence | Study design (each respondent = one row) |
...
differs across income levels (A4). If significant, run Tukey HSD.
Report F, p, η², and post-hoc results.
:::
...
and interaction term. Compute partial η² for each term.
:::
