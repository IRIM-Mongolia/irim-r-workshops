...
A chi-square test examines whether there is a statistically significant
association between two categorical variables. It compares the
frequencies you actually observe in your data against the frequencies
you would expect to see if the two variables were completely independent
of each other.

Chi-square tests are ideal for this dataset when both variables are
categorical (factors or logical). For example, we might ask:

**Is there an association between gender and which flexible working
policy goal a respondent selected?**
...
-   Observations are independent (each respondent appears only once)
-   Variables are categorical
-   Expected cell counts are \>= 5 in at least 80% of cells
-   No cell has an expected count of zero

### Example Research Question

Is there a significant association between area type (Rural vs Urban,
variable A6) and whether respondents selected 'Improve employee
wellbeing & work-life balance' as a policy goal (B1_1)?
