...
specifically [lesson
02-starting-with-data](https://datacarpentry.github.io/r-socialsci/02-starting-with-data.html),
and [R for
...
load the tidyverse and here packages. Let's do this in a new R notebook
that we'll save in our `scripts` folder. Save your R notebook with a
filename that makes sense, such as `starting_with_data.Rmd`.

Add a new code chunk, load the tidyverse and here packages, and read in
the SAFI dataset. We'll **assign** the dataset to an object called
`interviews`.
...
`here("data", "SAFI_clean.csv")` and `here("data/raw/SAFI_clean.csv")`
...
'double'),
``` respondent_wall_type`` is a column of characters ( ```<chr>`) and the`interview_date`is a column in the "date and time" format (`<dttm>\`).
...
other Carpentries lessons like the [Software Carpentry Programming with
